...
<nav class="bs-docs-sidebar  affix " >
	...
	<form action="<%=request.getContextPath()%>/order/buyprodend.action" method="post">
		<h2>当前用户</h2><br/>
		<!-- session用户名称 -->
		用户名
		<h2>选择地址</h2><br/>
	...
	<table border="0" class="gouwutable" cellspacing="0" cellpadding="0">
		...
		<tr class="prodRow">			
			<td class="prodcol1"><input type="hidden" name="orderId" value=""/>${orders.orderId}</td>
			<td class="prodcol2"><a href="javascript:">${orders.ordersMoney}</a> </td>
			<td class="prodcol3">
			<c:forEach items="${orders.goodsList}" var="goods">
			<img src="${goods.imgUrl}">
			<span class="xianjia">商品名称:${goods.goodName}  单价:${goods.goodPrice}</span>
			</c:forEach>
